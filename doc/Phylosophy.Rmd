...
empiricism<-df%>%filter(school=='empiricism')
rationalism<-df%>%filter(school=='rationalism')
german_idealism<-df%>%filter(school=='german_idealism')
nietzsche<-df%>%filter(school=='nietzsche')
feminism<-df%>%filter(school=='feminism')
```
Since there are too many schools in the dataset, I select 6 of them as representatives based on the number of observations each school talked about love, which are:   
**Plato, Aristotle, Empiricism, Rationalism, German Idealism, Nietzsche, and Feminism Philosophy.**   
...
The word frequency chart and word cloud of the three periods we have in our data are consistent with each other, and are shown below.
...
clean(contemporary)
```
We could see from the above results that

## IV. Obtain word frequency and word cloud of different schools
I try to find what philosophers say about love by counting the frequency of each words, plotting a word cloud to make it more intuitively, and comparing the results of different schools.

According to Aristotle philosophy, love is always talked along with friendship, which indicates that love and friendship may be a main topic learned by Aristotle philosophy, and in their opinion, love is a very pleasant thing.
```{r, warning=FALSE, message=FALSE, echo = FALSE}
clean(aristotle)
```

As for Plato, love is a beautiful and desirable thing. love is a desire for beauty. god
```{r, warning=FALSE, message=FALSE, echo = FALSE}
clean(plato)
```

In Emiricism philosophy, the most frequent word about love is hatred.
```{r, warning=FALSE, message=FALSE, echo = FALSE}
clean(empiricism)
```
...
```{r, warning=FALSE, message=FALSE, echo = FALSE}
clean(german_idealism)
```

Nietzsche philosophy always mention "life" when talking about "love".
```{r, warning=FALSE, message=FALSE, echo = FALSE}
clean(nietzsche)
```

It is not surprsing at all to find that the most frequent word for Frminism philosophy is woman, and there are several words that express the identities of different people.
```{r, warning=FALSE, message=FALSE, echo = FALSE}
clean(feminism)
```
...
Love is a very essential part of our life, and this is agreed by philosophers from all different schools and periods.
